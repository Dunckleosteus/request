mod geometry_converter;
use geometry_converter::open_file;
use polars::prelude::*;
use std::fs::File;
use std::io::prelude::*;
use std::{env, io};

use crate::geometry_converter::format_contents;
...
fn json2dataframe(path: &str) -> std::result::Result<polars::frame::DataFrame, PolarsError> {
    ...
    let dataset_length = dataset
        .as_array()
        .expect("Could not convert dataset to array")
        .len();
    // define vecs
